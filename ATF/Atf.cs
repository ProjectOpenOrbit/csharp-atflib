...
namespace Atf
{
    ...
    public class ATFReader
    {
        ...
        public ATFReader(Stream data)
        {
            ...
            if (atfStream.Length <= 20)
            {
                throw new Exception("Too small to be a valid ATF data");
            }

            using (BinaryReader reader = new BinaryReader(atfStream, Encoding.UTF8))
            {
                if (new string(reader.ReadChars(3)) != "ATF")
                {
                    throw new Exception("No ATF signature found");
                }
                ...
                byte[] image;
                if (_atfFormat == ATFFormat.ATFRAWCOMPRESSED)
                {
                    image = DxtUtil.DecompressDxt1(reader.ReadBytes((int)length), _width, _height);
                }
                else if (_atfFormat == ATFFormat.ATFRAWCOMPRESSEDALPHA)
                {
