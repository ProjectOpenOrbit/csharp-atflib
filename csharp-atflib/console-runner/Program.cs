...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        try
        {
            using (var atFileStream = new FileStream(atfname, FileMode.Open, FileAccess.Read))
            {
                atf = new ATFReader(atFileStream);
            }
        }
        catch (Exception e)
        {
        ...
        var nameWithoutExtension =
            Path.Combine(Path.GetDirectoryName(atfname), Path.GetFileNameWithoutExtension(atfname));
        ...
        if (!File.Exists(xmlFileName))
        {
            ...
            atf.Bitmap.Save(nameWithoutExtension + ".png", ImageFormat.Png);
            return;
        }
        ...
        if (!atlas.Correct)
        {
            Console.WriteLine("Incorrect atlas!");
            return;
        }
